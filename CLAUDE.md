...
21. **MinGW weak symbol conflicts**: MinGW on Windows doesn't support weak symbol override like GCC on Linux. If a function has both a weak stub in `generated_all_stubs.cpp` and a strong definition in `gpushare_client.cpp`, MinGW linker errors with "multiple definition". Remove the weak stub when adding a strong implementation. All install scripts auto-remove known conflicting stubs before building.
22. **TCP connect timeout**: The client's `TcpTransport::connect()` uses non-blocking connect with `select()` and a 5-second timeout. Without this, `::connect()` blocks 60-120 seconds on Windows when the server is unreachable, hanging Python startup (since our DLL connects during `cuInit`).
...
- **Local GPU passthrough**: When a local NVIDIA GPU exists, the client detects it via dlopen of real CUDA libraries from `/usr/local/lib/gpushare/real/` (Linux/macOS) or `C:\Program Files\gpushare\real\` (Windows, using LoadLibraryExA/GetProcAddress). Both local and remote GPUs are presented to applications. Default: device 0..N-1 = local, N+ = remote. With `remote_first=true`: device 0..N-1 = remote, N+ = local. `cudaSetDevice()` routes via `is_remote_device()` + `to_local_device()`/`to_remote_device()`. Config: `gpu_mode=all|remote|local`, `remote_first=true/false` in client.conf, or `GPUSHARE_GPU_MODE`/`GPUSHARE_REMOTE_FIRST` env vars. Requires `RTLD_DEEPBIND` on Linux. On Windows, DLL replacement is ALWAYS used (nvcuda.dll in torch\lib, cudart64_*.dll in Python directories). The C library handles all routing internally.
- **Remote priority mode**: `remote_first=true` in client.conf or `GPUSHARE_REMOTE_FIRST=1` makes remote GPUs device 0. This is the recommended approach for Windows dual-GPU — PyTorch defaults to device 0, so the powerful remote GPU is used automatically without any code changes or Python hook interception of tensor operations. All device-index-aware functions (cudaSetDevice, cudaGetDeviceProperties, cudaMalloc, NVML, etc.) use `is_remote_device()` + `to_local_device()`/`to_remote_device()` for translation.
- **Python startup hook**: `python/gpushare_hook.py` + `python/gpushare.pth` patches `torch.cuda` after import completes (using import-depth tracking to avoid circular imports). Two-phase GPU discovery with timeouts (5s ctypes in daemon thread, 3s TCP fallback). Patches 16 functions including `get_arch_list`, `_check_capability`, `is_available`, `device_count`, `set_device`, etc. Suppresses SM compatibility warnings. The hook is supplementary to the C library — the C library handles actual tensor operation routing, the hook fixes PyTorch's Python-level compatibility checks. No code changes needed in user applications.
